...
export default function MarcarCompletado({
  dia,
  completado,
  finDeFase,
}: {
  dia: number;
  completado: boolean;
  /** Número de fase si `dia` es el último día de esa fase — muestra una celebración extra al marcar. */
  finDeFase?: number;
}) {
  const [celebrando, setCelebrando] = useState(false);
  const [celebrandoFase, setCelebrandoFase] = useState(false);
  ...
  function celebrar() {
    ...
    if (finDeFase !== undefined) {
      setCelebrandoFase(true);
      window.setTimeout(() => setCelebrandoFase(false), 3000);
    }
  }
  ...
  return (
    <div>
      ...
      {celebrandoFase && (
        <p className="anim-rise mt-3 text-sm font-semibold text-ember-2">🏆 ¡Fase {finDeFase} completa!</p>
      )}
    </div>
  );
}
