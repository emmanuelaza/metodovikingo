...
Copia `schemas/leccionDiaria.ts` y `schemas/piezaMetodo.ts` a `schemaTypes/` del Studio y regístralos:

```ts
// schemaTypes/index.ts
import { leccionDiaria } from "./leccionDiaria";
import { piezaMetodo } from "./piezaMetodo";
export const schemaTypes = [leccionDiaria, piezaMetodo];
```
...
- 30 documentos `leccionDiaria` (días 1 a 30). Marca `esDiaDePieza` en los días 7, 14, 21 y 30.
- 4 documentos `piezaMetodo` (numero 1–4, dias 7/14/21/30).
